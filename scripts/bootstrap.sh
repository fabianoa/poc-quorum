...
function installgo {
  ...
  then
    ...
    nV2=$(echo $V2 | sed 's/\.//g')
    if (( $nV1 >= $nV2 )); then
      echo "Your current version of golang ($V1) is higher than the required to run Quorum nodes ($V2), so we will use it"
    else
...
function installquorum {
  ...
  then
    ...
    pushd /tmp
    wget https://github.com/jpmorganchase/quorum/archive/v2.1.1.tar.gz
